{ lib, config, pkgs,desktopSessionCmd ? "${pkgs.coreutils}/bin/true", ... }:
let
...
in {
	options.features.login.greetd = {
		...
		showTime = lib.mkOption {type = lib.types.bool; default = true; };
		remember = lib.mkOption {type = lib.types.bool; default = true; };
	};
